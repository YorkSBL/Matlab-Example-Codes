...
% ---- Eigenval params
sigma = 1e2; % {1e2}
numberEigenvalues = 16; % {16}
% ---- animation/video stuff
tP=90;  % # of time points/cycle
vidS=0;  % boolean to create movie file {0}
fRate= 20;          % frame rate [Hz]
vidName= 'memb2D.avi';
% ====================================
...
tri=delaunay(x,y);
h3 = figure(3); clf;
subset= [1 2 4 6 7 9 11 13 15];
titles={'(0,1)','(1,1)','(2,1)','(0,2)','(3,1)','(1,2)','(4,1)','(2,2)','(0,3)'};
% ----
if vidS==1
    % -----
    % create AVI object
...
for m=1:tP
    for n=1:numel(subset)
        nn= subset(n);
        zval= eigenvectorsPde(:,nn);
        z= zval*cos(2*pi*m/tP);
        subplot(3,3,n);
        trisurf(tri,x,y,z); view([0 45]); title(titles(n));
        axis off; grid off; colormap jet;
    ...
    pause(0.01);
    if (vidS==1), writeVideo(vidObj, getframe(h3)); end 
end
